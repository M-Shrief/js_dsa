...
export function quickSort(list: number[], direction: DIRECTION, start?: number, end?: number): number[] {
    start = start ?? 0;    
...
function partition(list: number[], direction: DIRECTION, start: number, end: number): number {
    let pivot = list[end];
