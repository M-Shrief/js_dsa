...
export class Heap<T> {
  ...
  private down(parent: number) {
    ...
    if (
      leftChild < this.list.length &&
      this.compareFn(this.list[leftChild], this.list[current])
    ) {
      current = leftChild;
    }

    if (
      rightChild < this.list.length &&
      this.compareFn(this.list[rightChild], this.list[current])
    ) {
      current = rightChild;
    }
